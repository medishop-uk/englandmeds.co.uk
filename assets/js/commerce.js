...
var DATA_API_URL=window.ENGLANDMEDS_DATA_API||'';
var DATA_SPREADSHEET_ID=window.ENGLANDMEDS_DATA_SPREADSHEET_ID||'';
var pricing={
...
var cart=[];
try{cart=JSON.parse(localStorage.getItem('englandmedsCart')||'[]')}catch(error){cart=[]}
function money(value){return '£'+Number(value).toFixed(0)}
function saveCart(){localStorage.setItem('englandmedsCart',JSON.stringify(cart));renderCart();updateCount()}
